  ...
  WHERE Id = {item_id}'''
      ...
      with sqlite3.connect(conn_string) as conn:
        ...
        try:
          ...
          conn.rollback()
          raise BadRequest(f'Invalid request - data does not match table schema: {e} query:{query} itemid:{item_id} clause:{set_clause}')
      return {
        # 'data': data,
        # 'token': headers.get('X-RequestDigest'),
        # 'table': check_reqs.get('table'),
...
@api_namespace.route(
    "/web/GetFolderByServerRelativeUrl('<folder_name>')/Files('<file_name>')/$value",
    doc={'description': '''Endpoint for updating List items.'''})
@api_namespace.doc(params={
    'folder_name': 'Simulated SP folder id',
    'file_name': 'name of file in simulated sp'
})
class GetFiles(Resource):
  def get(self, folder_name, file_name):
    with open("/usr/src/project/data/gadm_sg_400nm_country_only_4dp.txt","r") as f:
      data = json.load(f)
    print(folder_name)
    print(file_name)
    return data
